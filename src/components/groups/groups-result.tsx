...
export function GroupsResult({ school: schoolSlug, level, course }: { school: string; level: string; course: string }) {
  const school = getSchool(schoolSlug)
  const currentCourse = getCourse(schoolSlug, level, course)
  const currentLevel = getLevel(level)
  if (!school || !currentCourse || !currentLevel) notFound()
  ...
  return (
    <main className="mx-auto flex min-h-svh w-full min-w-0 max-w-7xl flex-col gap-8 px-6 py-52 md:gap-0">
      ...
      <div className="flex flex-row gap-3 md:mt-25.75">
        <CardCourseGroup groupName={`Gruppo Generale ${school.name}`} stacked />
